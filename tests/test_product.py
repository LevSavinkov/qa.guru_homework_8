...
class TestProducts:
    ...
    def test_book_check_quantity(self, product):
        book = product
    ...
    def test_book_check_quantity_negative(self, product):
        book = product
        with pytest.raises(ValueError) as err:
            book.check_quantity(-1)
        assert str(err.value) == NEGATIVE_NUMBER_ERROR
    
    def test_book_buy(self, product):
        book = product
        # TODO напишите проверки на метод buy
        assert book.buy(0) == 1000
        assert book.buy(1000) == 0
    
    def test_book_buy_negative(self, product):
        book = product
        with pytest.raises(ValueError) as err:
            book.buy(-1)
        assert str(err.value) == NEGATIVE_NUMBER_ERROR
    
    def test_book_buy_more_than_available(self, product):
        book = product
        # TODO напишите проверки на метод buy,
